...
class EmailVerificationNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->greeting(trans('accounts::auth.register.verification.greeting', [
                'user' => $notifiable->email,
            ]))
            ->subject(trans('accounts::auth.register.verification.subject'))
            ->line(trans('accounts::auth.register.verification.line', [
                'code' => $this->code,
            ]))
            ->line(trans('accounts::auth.register.verification.time', [
                'minutes' => Verification::EXPIRE_DURATION / 60,
            ]))
            ->line(trans('accounts::auth.register.verification.footer'))
